...
# KERNEL
run_thor_40:
	srun --ntasks 1 --cpus-per-task 16 --partition=A100 --gpus=3g.40gb:1 -t 9:00:00 --pty bash

run_thor_20:
	srun --ntasks 1 --cpus-per-task 16 --partition=A100 --gpus=1g.20gb:1 -t 9:00:00 --pty bash

run_baldur:
	srun --ntasks 1 --cpus-per-task 16 --partition=RTX --gpus=2080ti:1 -t 9:00:00 --pty bash

start_jup:
	jupyter notebook --no-browser --port 40001 --NotebookApp.allow_origin='*' --NotebookApp.ip='0.0.0.0'
